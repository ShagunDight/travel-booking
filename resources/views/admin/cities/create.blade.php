        ...
        <div class="card-body">
            <form id="cityForm" action="{{ route('admin.cities.store') }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="state" class="form-control rounded-3 shadow-sm" value="{{ old('state') }}">
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Country</label>
                    <select name="country" id="country" class="form-select rounded-3 shadow-sm">
                        <option value="">-- Select Country --</option> 
                        @foreach($countries as $country) 
                            <option value="{{ $country->country_code }}" {{ old('country') == $country->country_code ? 'selected' : '' }}> 
                ...
                <div class="mb-3">
                    ...
                    <select name="emoji" class="form-select rounded-3 shadow-sm">
                        ...
                        @foreach($emojiList as $emoji)
                            <option value="{{ $emoji }}" {{ old('emoji') == $emoji ? 'selected' : '' }}>
                                {{ $emoji }}
                            </option>
                        @endforeach
                    </select>
