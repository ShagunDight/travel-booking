...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
        <h2 class="fw-bold mb-2">Add Room</h2>
    ...
    <div class="card shadow-sm rounded-4 border-0">
        <div class="card-body">
            ...
            <form id="roomForm" action="{{ route('admin.rooms.store') }}" method="POST" class="needs-validation" novalidate>
                @csrf
                <div class="mb-3">
                    ...
                    <select name="property_id" class="form-select rounded-3 shadow-sm @error('property_id') is-invalid @enderror" required>
                        ...
                        @foreach($hotels as $hotel)
                            <option value="{{ $hotel->id }}" {{ old('property_id') == $hotel->id ? 'selected' : '' }}>
                                {{ $hotel->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="capacity" class="form-control rounded-3 shadow-sm @error('capacity') is-invalid @enderror" value="{{ old('capacity',2) }}" min="1" required>
                    @error('capacity')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-end mt-4">
                    <button type="submit" class="btn btn-success btn-lg rounded-3 shadow-sm"><i class="bi bi-check-circle me-1"></i> Save </button>
                </div>
            </form>
